...
Launch 8 **background subagents** using `run_in_background: true` on the Task tool with `subagent_type: Explore`, one for each principle. The lead compiles the scored summary report after all agents complete.
